...
description: 编写可执行的应用代码（新功能 / bug 修复 / 重构）时使用，在实现代码之前。不适用于 Markdown 文档、配置文件、OpenSpec artifacts（proposal/design/specs/tasks/pre_design）、CLAUDE.md / SKILL.md / commands 等文本类编辑。
---
...
**始终使用（无需询问）：**
- 新功能 / Bug 修复 / 重构 / 行为变更（指**可执行的应用代码**）

**不适用（直接跳过 TDD，无需询问）：**
- Markdown 文档 / README / 注释
- 配置文件（json / yaml / toml / env，且非可执行 DSL）
- OpenSpec artifacts：`proposal.md` / `design.md` / `pre_design.md` / `specs/` / `tasks.md`
- Skill / Command / Hook / CLAUDE.md 等指令类文本
- 一次性脚本 / 探索性原型（用完即弃）
...
落入"始终使用"时，想着"就这一次跳过 TDD"？停下来。那是在给自己找借口。
...
编写新测试、设计测试结构、决定 mock 策略、判断"测多少够了"时，阅读 @testing-best-practices.md：
- AAA / Given-When-Then 结构（一个测试一个 Act）
- Test Doubles 优先级：Real > Fake > Stub > Spy > Mock
- 不 mock 你拥有的东西（只 mock 外部依赖）
- 覆盖以"行为"为单位（spec Scenario + happy path + 主要错误路径，覆盖完即停）
- 同一行为多输入变体用参数化
- 测试文件镜像源代码结构
