...
系统 SHALL 在 Obsidian 库中提供可由脚本初始化的长期知识库入口，使用户能够从每日分析入口进入公司、主题、事件、证据和复盘相关页面。

#### Scenario: 初始化长期知识库入口
- **WHEN** 使用日期 `2026-05-10` 初始化包含股票 `炼石航空`、代码 `000697.XSHE` 的分析目录
- **THEN** 系统 MUST 生成或保留可链接到公司、主题、事件、证据和复盘入口的 Obsidian Markdown 页面
...
系统 SHALL 提供主题与事件沉淀入口，使跨日期、跨公司的研究材料可以通过 Obsidian wikilink 被持续关联。

#### Scenario: 生成主题和事件入口
- **WHEN** 初始化任意日期的分析目录
- **THEN** 系统 MUST 生成或保留主题入口和事件入口 Markdown 页面，并包含可供后续追加研究材料的章节骨架

#### Scenario: 主题和事件入口使用 Obsidian 元数据
- **WHEN** 读取主题入口和事件入口 Markdown 页面
